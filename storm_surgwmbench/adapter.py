"""SurgWMBench train/eval adapter for the STORM baseline."""
...
from storm_surgwmbench.data import SurgWMBenchClipDataset, collate_dense_variable_length, collate_sparse_anchors
from storm_surgwmbench.evaluation import metrics as traj_metrics

BASELINE_KEY = "storm"
MODEL_NAME = "SurgWMBenchStormStochasticTransformer"
...
class StochasticVisualEncoder(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        hidden = self.backbone(images).flatten(1)
        mu = self.mu(hidden)
        logstd = self.logstd(hidden).clamp(min=-6.0, max=2.0)
        if self.training:
...
class FrameDecoder(nn.Module):
    ...
    def forward(self, z: torch.Tensor, size_hw: tuple[int, int]) -> torch.Tensor:
        recon = self.decoder(self.fc(z).view(z.shape[0], 128, 4, 4))
        if recon.shape[-2:] != size_hw:
            recon = F.interpolate(recon, size=size_hw, mode="bilinear", align_corners=False)
        return recon


class SurgWMBenchStormTransformer(nn.Module):
    """STORM-style stochastic latent encoder and Transformer world model."""

    def __init__(self, latent_dim: int = 64, hidden_dim: int = 128, num_layers: int = 2, num_heads: int = 4) -> None:
        super().__init__()
        self.encoder = StochasticVisualEncoder(latent_dim)
        ...
        self.coord_head = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ELU(inplace=True), nn.Linear(hidden_dim, 2))
    ...
    def forward(self, frames: torch.Tensor, frame_indices: torch.Tensor, num_frames: torch.Tensor, mask: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        ...
        return {"coords": coords, "recon": recon, "mu": mu.view(bsz, timesteps, -1), "logstd": logstd.view(bsz, timesteps, -1)}
...
def training_loss(output: dict[str, torch.Tensor], batch: dict[str, Any], config: AdapterConfig) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    kl = -0.5 * (1.0 + 2.0 * output["logstd"] - output["mu"].square() - (2.0 * output["logstd"]).exp()).mean()
...
def train_adapter(args: Any) -> dict[str, Any]:
    ...
    model = SurgWMBenchStormTransformer(config.latent_dim, config.hidden_dim, config.num_layers, config.num_heads).to(device)
...
def load_model_from_checkpoint(checkpoint: str | Path, device: torch.device) -> tuple[SurgWMBenchStormTransformer, AdapterConfig, dict[str, Any]]:
