...
The loader follows the public SurgWMBench manifest contract. It uses official
manifest files only, never creates random splits, and keeps sparse human anchor
labels separate from dense pseudo coordinates.
"""
...
def resolve_dataset_path(dataset_root: str | Path, value: str | Path | None) -> Path | None:
    """Resolve a dataset-relative path while preserving stored path strings."""
...
class SurgWMBenchClipDataset(Dataset):
    """Load SurgWMBench clips from final-layout manifests."""
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        frame_records = self._frame_records(annotation)
        ...
        selected_indices = self._select_indices(num_frames, anchor_local_indices, index)
        selected_frame_paths = self._paths_for_indices(frame_records, selected_indices)
    ...
    def _frame_records(self, annotation: dict[str, Any]) -> list[Any]:
        frames = annotation.get("frames")
    ...
    def _load_dense_coordinates(
        ...
        if self.strict:
            self._check_version(interpolation.get("dataset_version"), f"Interpolation {interpolation_path}")
    ...
    def _validate_dense_anchors(
        ...
        for anchor_pos, local_idx in enumerate(anchor_indices):
            ...
            if not np.isclose(float(dense["confidence"][local_idx].item()), 1.0):
                raise ValueError(f"{interpolation_path}: anchor local_frame_idx={local_idx} confidence is not 1.0")
            if not np.isclose(float(dense["label_weights"][local_idx].item()), 1.0):
    ...
    def _select_indices(self, num_frames: int, anchor_indices: list[int], sample_index: int) -> list[int]:
        if self.frame_sampling == "sparse_anchors":
        ...
        if self.frame_sampling == "window":
            if self.max_frames is None or self.max_frames <= 0 or num_frames <= self.max_frames:
                return list(range(num_frames))
            window = int(self.max_frames)
            max_start = num_frames - window
            start = (int(sample_index) * 9973) % (max_start + 1)
            return list(range(start, start + window))
        raise ValueError(f"Unsupported frame_sampling={self.frame_sampling!r}")

    def _paths_for_indices(self, frames: list[Any], indices: list[int]) -> list[Path]:
        ...
        for fallback_idx, frame in enumerate(frames):
            ...
            if value is None:
                if self.strict:
                    raise ValueError(f"Frame entry for local_frame_idx={local_idx} is missing frame_path")
                continue
        ...
        for local_idx in indices:
            if local_idx not in by_index:
                raise ValueError(f"No frame path listed for local_frame_idx={local_idx}")
...
class SurgWMBenchFrameDataset(Dataset):
    """Frame-level dataset for categorical VAE pretraining."""
    ...
    def __init__(
        ...
        for clip_idx, entry in enumerate(self.clip_dataset.entries):
            ...
            frame_records = self.clip_dataset._frame_records(annotation)
            num_frames = int(entry.get("num_frames", annotation.get("num_frames", len(frame_records))))
            paths = self.clip_dataset._paths_for_indices(frame_records, list(range(num_frames)))
